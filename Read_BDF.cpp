...
class read_bdf_data{
	...
	void read_data_complement_2(int **,int,int);
	void read_triggers(int *,int);
...
void read_bdf_data::read_data_complement_2(int **matrix_data_c2,int rows_data,int columns_data){
...
//Extract the latency and the trigger code for each event	
	void read_bdf_data::read_triggers(int *trig_vect, int length_trig_vect){
	...
	matrix_trig = new int *[length_trig_vect];
	 ...
	 matrix_trig[count] = new int [2];}
		...
		int *temp_first_8b = new int[length_trig_vect];	//Array to store bits 0 - 7;
		int *temp_second_8b = new int[length_trig_vect];	//Array to store bits 8 - 15;
		int *temp_third_8b = new int[length_trig_vect];	//Array to store bits 16 - 23. These bits are the status bits;
...
void read_bdf_data::read_data_bdf(FILE *file_data_f,int sf,int chan_n, int time_data){
	...
	int temp_data; 	//Variable to use to temporarly store a byte;
	int step_data; 
	 
	 //Dynamical allocation of the matrix used to store the data;
	matrix_data = new int *[temp_n_chan];
	 for (int count = 0; count < temp_n_chan; count++){
	 matrix_data[count] = new int [col_data];}
	 
	 //Dynamical allocation of the vector used to store the trigger;
	 vect_trig = new int [col_data];
	 //for (int count = 0; count < temp_n_chan; count++){
...
string name_bdf;
char name_bdf_char[name_bdf.size() + 1];

cout << "Type the name of the bdf file that you want to open: ";
